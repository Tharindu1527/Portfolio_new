        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {projects.map((p, i) => (
            <motion.article
              ...
              className={`panel group relative overflow-hidden rounded-lg p-6 transition-colors hover:border-[color:var(--color-accent)]/40 ${
                p.featured ? "md:col-span-1 ring-1 ring-[color:var(--color-accent)]/15" : ""
              }`}
            >
              {p.featured && (
                <div className="absolute right-4 top-4 flex items-center gap-1 font-mono text-[10px] uppercase tracking-widest text-[color:var(--color-accent-3)]">
              ...
              <h3 className="text-lg font-semibold leading-snug text-slate-100 pr-16">
                {p.title}
              </h3>
